...
export function StartOfDayView() {
  ...
  return (
    <div className={styles.container}>
      <h2 className={styles.title}>☀️ Start My Day</h2>
      <p className={styles.subtitle}>
        Generate a prompt for Copilot CLI to kick off your morning — review
        meetings, catch up on messages, and plan your focus time.
      </p>
      ...
      <ul className={styles.checklist}>
        ...
        <li className={styles.checkItem}>
          <span className={styles.checkIcon}>💬</span>
          <div className={styles.checkLabel}>
            <strong>Teams & Email Check</strong>
            <span>Find messages that need a response</span>
          </div>
        </li>
        <li className={styles.checkItem}>
      ...
      {status === 'copied' && (
        <div className={`${styles.status} ${styles.success}`}>
          ✓ Prompt copied! Paste it into Copilot CLI to start your morning workflow.
        </div>
      )}
      {status === 'error' && (
